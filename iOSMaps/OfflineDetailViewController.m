...
/* Returns a new array consisted of MAOfflineCity object for which match the
 * key. */
...
- (NSString *)cellDetailTextForItem:(MAOfflineItem *)item {
  ...
  if (![self.downloadingItems containsObject:item]) {
    if (item.itemStatus == MAOfflineItemStatusCached) {
      detailText = [NSString
          stringWithFormat:@"%lld/%lld", item.downloadedSize, item.size];
    } else {
    ...
    MAOfflineMapDownloadStatus status =
        [[stage objectForKey:DownloadStageStatusKey2] intValue];
    ...
    switch (status) {
      ...
      case MAOfflineMapDownloadStatusProgress: {
        ...
        long long recieved = [[progressDict
            objectForKey:MAOfflineMapDownloadReceivedSizeKey] longLongValue];
        long long expected = [[progressDict
            objectForKey:MAOfflineMapDownloadExpectedSizeKey] longLongValue];

        detailText =
            [NSString stringWithFormat:@"%lld/%lld(%.1f%%)", recieved, expected,
                                       recieved / (float)expected * 100];
        break;
      }
...
- (void)updateAccessoryViewForCell:(UITableViewCell *)cell
                              item:(MAOfflineItem *)item {
  UIButton *delete = nil;
  ...
  CGPoint right =
      CGPointMake(kButtonSize * (kButtonCount - 0.5), kButtonSize * 0.5);
  CGFloat leftMove = -kButtonSize;
  CGPoint center = right;
  if (item.itemStatus == MAOfflineItemStatusInstalled ||
      item.itemStatus == MAOfflineItemStatusCached) {
    delete.hidden = NO;
...
- (void)updateUIForItem:(MAOfflineItem *)item
  ...
  if ([item isKindOfClass:[MAOfflineItemCommonCity class]]) {
    UITableViewCell *provinceCell = [self.tableView
        cellForRowAtIndexPath:[NSIndexPath indexPathForRow:0
                                                 inSection:indexPath.section]];
    if (provinceCell != nil) {
      [self updateCell:provinceCell
               forItem:((MAOfflineItemCommonCity *)item).province];
    }
    return;
  }
  ...
  if ([item isKindOfClass:[MAOfflineProvince class]]) {
    ...
    [province.cities enumerateObjectsUsingBlock:^(MAOfflineItemCommonCity *obj,
                                                  NSUInteger idx, BOOL *stop) {
      UITableViewCell *cityCell = [self.tableView
          cellForRowAtIndexPath:[NSIndexPath
                                    indexPathForRow:idx + 1
                                          inSection:indexPath.section]];
...
- (void)download:(MAOfflineItem *)item atIndexPath:(NSIndexPath *)indexPath {
  ...
  [[MAOfflineMap sharedOfflineMap]
      ...
      shouldContinueWhenAppEntersBackground:YES
                              downloadBlock:^(
                                  MAOfflineItem *downloadItem,
                                  MAOfflineMapDownloadStatus downloadStatus,
                                  id info) {

                                if (![self.downloadingItems
                                        containsObject:downloadItem]) {
                                  [self.downloadingItems
                                      addObject:downloadItem];
                                  [self.downloadStages
                                      setObject:[NSMutableDictionary dictionary]
                                         forKey:downloadItem.adcode];
                                }

                                /* Manipulations to your application’s user
                                 * interface must occur on the main thread. */
                                dispatch_async(dispatch_get_main_queue(), ^{

                                  NSMutableDictionary *stage =
                                      [self.downloadStages
                                          objectForKey:downloadItem.adcode];

                                  if (downloadStatus ==
                                      MAOfflineMapDownloadStatusWaiting) {
                                    [stage
                                        setObject:[NSNumber numberWithBool:YES]
                                           forKey:DownloadStageIsRunningKey2];
                                  } else if (
                                      downloadStatus ==
                                      MAOfflineMapDownloadStatusProgress) {
                                    [stage setObject:info
                                              forKey:DownloadStageInfoKey2];
                                  } else if (
                                      downloadStatus ==
                                    ...
                                    // clear
                                    [self.downloadingItems
                                        removeObject:downloadItem];
                                    [self.downloadStages
                                        removeObjectForKey:downloadItem.adcode];
                                  }

                                  [stage setObject:[NSNumber numberWithInt:
                                                                 downloadStatus]
                                            forKey:DownloadStageStatusKey2];
...
- (MAOfflineItem *)itemForIndexPath:(NSIndexPath *)indexPath {
  ...
  switch (indexPath.section) {
    ...
    default: {
      MAOfflineProvince *pro =
          self.provinces[indexPath.section - self.sectionTitles.count];
...
- (UIButton *)buttonWithImage:(UIImage *)image tag:(NSUInteger)tag {
  UIButton *button = [[UIButton alloc]
      initWithFrame:CGRectMake(0, 0, kButtonSize, kButtonSize)];
  [button setImage:image forState:UIControlStateNormal];
  button.tag = tag;
  button.center =
      CGPointMake((kButtonCount - tag + 0.5) * kButtonSize, kButtonSize * 0.5);
...
- (UIView *)accessoryView {
  UIView *accessory = [[UIView alloc]
      initWithFrame:CGRectMake(0, 0, kButtonSize * kButtonCount, kButtonSize)];
  UIButton *downloadButton =
      [self buttonWithImage:[self downloadImage] tag:kTagDownloadButton];
  UIButton *pauseButton =
      [self buttonWithImage:[self pauseImage] tag:kTagPauseButton];
  UIButton *deleteButton =
      [self buttonWithImage:[self deleteImage] tag:kTagDeleteButton];
...
- (CGFloat)tableView:(UITableView *)tableView
  ...
  return section < self.sectionTitles.count ? kSectionHeaderHeight
                                            : kTableCellHeight;
}
  ...
  switch (section) {
    ...
    default: {
      if (_expandedSections[section]) {
        MAOfflineProvince *pro =
            self.provinces[section - self.sectionTitles.count];
...
- (UIView *)tableView:(UITableView *)tableView
  ...
  if (section < self.sectionTitles.count) {
    ...
    UILabel *lb = [[UILabel alloc]
        initWithFrame:CGRectMake(kSectionHeaderMargin, 0,
...
- (UITableViewCell *)tableView:(UITableView *)tableView
  ...
  UITableViewCell *cell =
      [tableView dequeueReusableCellWithIdentifier:cityCellIdentifier];
...
- (void)searchAction {
  /* 搜索关键字支持 {城市中文名称, 拼音(不区分大小写), 拼音简写, cityCode,
   * adCode}五种类型. */
  ...
  [result enumerateObjectsUsingBlock:^(MAOfflineCity *obj, NSUInteger idx,
                                       BOOL *stop) {
    NSLog(@"idx = %d, cityName = %@, cityCode = %@, adCode = %@, pinyin = %@, "
          @"jianpin = %@, size = %lld",
          (int)idx, obj.name, obj.cityCode, obj.adcode, obj.pinyin, obj.jianpin,
          obj.size);
  }];
}
...
- (void)initToolBar {
  ...
  prompts.text = [NSString
      stringWithFormat:@"默认关键字是\"%@\", 结果在console打印",
                       kDefaultSearchkey];
  ...
  UIBarButtonItem *promptsItem =
      [[UIBarButtonItem alloc] initWithCustomView:prompts];
  ...
  self.toolbarItems =
      [NSArray arrayWithObjects:flexbleItem, promptsItem, flexbleItem,
                                searchItem, flexbleItem, nil];
}
...
- (void)setupCities {
  ...
  _expandedSections = (char *)malloc(
      (self.sectionTitles.count + self.provinces.count) * sizeof(char));
  memset(_expandedSections, 0,
         (self.sectionTitles.count + self.provinces.count) * sizeof(char));
}
...
- (void)setupPredicate {
  self.predicate = [NSPredicate
      predicateWithFormat:@"name CONTAINS[cd] $KEY OR cityCode CONTAINS[cd] "
                          @"$KEY OR jianpin CONTAINS[cd] $KEY OR pinyin "
                          @"CONTAINS[cd] $KEY OR adcode CONTAINS[cd] $KEY"];
}
...
- (NSArray *)citiesFilterWithKey:(NSString *)key {
  ...
  NSPredicate *keyPredicate = [self.predicate
